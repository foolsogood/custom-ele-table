class Tools {
  public convert2Zero(a: any) {
    return Object.is(Number(a), NaN) ? 0 : a;
  }
  //浮点型加法函数
  public floatAdd(a: number, b: number) {
    let r1;
    let r2;
    try {
  ...
  //浮点型乘法
  public floatMul(a: number, b: number) {
    let c = 0;
    const d = a.toString();
    const e = b.toString();
    try {
  ...
  //浮点型除法
  public floatDiv(a: number, b: number) {
  ...
  //节流
  public throttle(fn: () => void, delay: number) {
    let endTime: number;
    let timer: any;
    const self = this;
  ...
  public checkIfObjectEqual(objA: any, objB: any): boolean {
    // tslint:disable-next-line: only-arrow-functions
    const isType = function(obj: any) {
